...
replicate_df <- df %>%
  pivot_longer(cols=c(FT, SEED_WEIGHT_100, FECUNDITY), names_to='PHENOTYPE', values_to="VALUE") %>%
...
replicate_df <- add_generation(replicate_df)

replicate_df %>% filter(PHENOTYPE=="FECUNDITY") %>%
...
ggsave("derived_trait_replicate_correlation_plots.png", raf)


## how to filter...
## based on a verylow correlation of total mass between replicates within-year
# calculate difference between R1 and R2
replicate_df <- replicate_df %>% filter(PHENOTYPE=="FECUNDITY"|PHENOTYPE=="SEED_WEIGHT_100")
...
tm_filt <- replicate_df %>% filter(PHENOTYPE=="FECUNDITY") %>% filter(rep_diff<100 & rep_diff>-100)
problem_reps_tm <- replicate_df %>% filter(PHENOTYPE=="FECUNDITY") %>% filter(rep_diff>100 | rep_diff< -100)
