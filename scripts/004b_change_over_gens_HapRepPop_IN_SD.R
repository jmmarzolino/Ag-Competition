...
hap <- hap %>% mutate(across(-c(Generation), ~(scale(.) %>% as.vector)))
...
write_delim(normality_table, "happop_blup_normality_pvals_IN_SD.tsv", "\t")
...
write_delim(var_tab_sig, "happop_traits_var_change_sig_test_IN_SD.tsv", "\t")
...
write_delim(tmp, "happop_traits_var_diff_btwn_gens_sig_IN_SD.tsv", "\t")
...
write_delim(tmp, "happop_blups_anova_posthoc_results_IN_SD.tsv", "\t")
