...
#SBATCH --output=/rhome/jmarz001/bigdata/Ag-Competition/scripts/003b_change_over_gens_in_sd.stdout
#SBATCH -p koeniglab
...
df <- df %>% mutate(across(-c(Generation), ~(scale(.) %>% as.vector)))
...
write_delim(var_tab_sig, "traits_var_change_sig_test_IN_SD.tsv", "\t")
...
write_delim(tmp, "traits_var_diff_btwn_gens_sig_IN_SD.tsv", "\t")
...
write_delim(tmp, "blups_anova_posthoc_results_IN_SD.tsv", "\t")
