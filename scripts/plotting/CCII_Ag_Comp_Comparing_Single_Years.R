...
correct <- c("Genotype", "Condition", "Generation", "TOTAL_WEIGHT", "100 seed weight", "Fecundity", "Fitness", "FT", "Haplotype", "Atlas_Avg_Fec", "Atlas_Avg_Fitness", "Atlas_Avg_Total_Weight", "Centered_Fit", "Centered_FT", "Centered_Fec", "Centered_TW", "Exp_Fec_Per_Plant",
...
Combined_Single_Years_Distribution <- Combined_Single_Years_Distribution %>% mutate(Average_TW = mean(TOTAL_WEIGHT),
                                                                                                       Average_FT = mean(FT),
...
### TW
ggplot(Combined_Single_Years, aes(TOTAL_MASS_2021_2022, TOTAL_WEIGHT), add = "reg.line") +
  geom_point() +
...
ggplot(Combined_Single_Years_Distribution, aes(x = TOTAL_WEIGHT, fill = Year)) +
