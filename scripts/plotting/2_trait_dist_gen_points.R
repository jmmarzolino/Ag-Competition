...
#SBATCH --output=/rhome/jmarz001/bigdata/Ag-Competition/scripts/plotting/2_trait_dist_gen_points.stdout
#SBATCH -p koeniglab
...
df_rnd$FITNESS <- round(df$FITNESS)
...
# fitness
F18 <- as.numeric(names(sort(table(df_rnd[which(df_rnd$Generation==18), which(colnames(df_rnd)=="FITNESS")]), decreasing=T)[1:5]))
F58 <- as.numeric(names(sort(table(df_rnd[which(df_rnd$Generation==58), which(colnames(df_rnd)=="FITNESS")]), decreasing=T)[1:5]))
FIT_cmn <- tibble(F18, F58) %>% pivot_longer(names_to="Generation", cols=everything())
...
d <- 
ggplot() +
  geom_density(data=df, aes(FITNESS), linewidth=1) +
  geom_vline(data=FIT_cmn, aes(xintercept=value, color=Generation), linewidth=1) +
  scale_color_manual(values=c("#6BAED6", "#084594")) + 
  labs(x="", y="density", title="Fitness Distribution", subtitle="with most common values colored") +
  theme_bw(base_size=18) +
  guides(color = "none")
