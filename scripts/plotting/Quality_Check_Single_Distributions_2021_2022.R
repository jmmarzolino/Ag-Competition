...
tmp <- Single_2021_2022 %>% group_by(Generation) %>% summarise(Avg_TW = mean(TOTAL_MASS)) %>% ungroup() 
...
ANOVA_TW <- aov(TOTAL_MASS ~ as.factor(Generation), Single_2021_2022)
summary(ANOVA_TW)
TukeyHSD(ANOVA_TW)
...
T <- Single_2021_2022 %>% select(TOTAL_MASS, Generation, Genotype) %>%  arrange(Generation)
...
tmp <- Single_2021_2022 %>% group_by(Generation) %>% summarise(Avg_FT = mean(FT)) %>% ungroup() 
graph_tmp <- full_join(Single_2021_2022, tmp, by = "Generation")
...
ANOVA_FT <- aov(FT ~ as.factor(Generation), Single_2021_2022)
summary(ANOVA_FT)
TukeyHSD(ANOVA_FT)
...
T <- Single_2021_2022 %>% select(FT, Generation, Genotype) %>%  arrange(Generation)
T <- T %>% mutate(FT = FT)
...
tmp <- Single_2021_2022 %>% select(c(Generation, Genotype, Fecundity)) %>% group_by(Generation) %>% summarise(Avg_Fec = mean(Fecundity, na.rm = TRUE)) %>% ungroup()
graph_tmp <- full_join(Single_2021_2022, tmp, by = "Generation")
...
T <- Single_2021_2022 %>% select(Fecundity, Generation, Genotype) %>%  arrange(Generation)
...
dunn.test(Single_2021_2022$FEC, g = Single_2021_2022$Generation)
...
T <- Single_2021_2022 %>% select(Fitness, Generation, Genotype) %>%  arrange(Generation)
