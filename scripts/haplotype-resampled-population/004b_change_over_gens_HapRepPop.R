...
x4c <- rownames(x4)
x4 <- tibble(x4c, x4)
colnames(x4) <- c("trait", "F0_F18_vardiff", "F18_F58_vardiff")


##########   are var differences significant?   #########
var_tab <- tibble("trait"=collist, "all_gens"=as.numeric(NA), "F0_F18"=as.numeric(NA), "F18_F58"=as.numeric(NA)) 
...
var_tab_sig <- var_tab_sig %>% pivot_longer(cols=c(F0_F18, F18_F58), names_to="generations_compared", values_to="Levene_pval")
var_tab_sig$Levene_sig <- var_tab_sig$Levene_pval < 0.05


tmp <- var_tab_sig %>% 
  pivot_longer(cols=c(F0_F18_vardiff, F18_F58_vardiff), names_to="gensvardiff", values_to="var_difference") 
...
signif_tab <- tibble("trait"=collist, "F0_F18"=as.numeric(NA), "F18_F58"=as.numeric(NA)) 
...
signif_tab_KW <- signif_tab %>% pivot_longer(cols=c(F0_F18, F18_F58), names_to="generations_compared", values_to="Kruskal_pval")
signif_tab_KW$Kruskal_sig <- signif_tab_KW$Kruskal_pval < 0.05

# set up storing posthoc test results
signif_tab$F0_F18_pval <- as.numeric(NA) 
...
x4c <- rownames(x4)
x4 <- tibble(x4c, x4)
colnames(x4) <- c("trait", "F0_F18_meandiff", "F18_F58_meandiff")
...
signif_tab2 <- signif_tab %>% pivot_longer(cols=c(F0_F18, F18_F58), names_to="generations_compared", values_to="Kruskal_pval")
signif_tab2$Kruskal_sig <- signif_tab2$Kruskal_pval < 0.05


tmp <- signif_tab2 %>% 
  pivot_longer(cols=c(F0_F18_meandiff, F18_F58_meandiff), names_to="gensmeandiff", values_to="mean_difference") %>%
  pivot_longer(cols=c(F0_F18_pval, F18_F58_pval), names_to="genspostpval", values_to="Dunn_posthoc_pval")
